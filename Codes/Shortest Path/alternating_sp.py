...
def alt_omegafix(filename, time_limit, duals, omega0, real_cost, env):
...
def alt_dualesfix(filename, time_limit, duals, omega0, costoreal, env):
...
def iter_alt(iterations, filename, time_limit, omega0, real_cost, total_time, env):
    
    t0 = time.time()
    
    #Initial solution
    alt = alt_omegafix(filename, time_limit, [], omega0, real_cost, env)
    ...
    for i in range(iterations):
        if i % 2 == 0:
            alt = alt_dualesfix(filename, time_limit, alt[0], alt[1], real_cost, env)
            time_iter_dual.append(alt[3])
            
        else:
            alt = alt_omegafix(filename, time_limit, alt[0], alt[1], real_cost, env)
            time_iter_omega.append(alt[3])
