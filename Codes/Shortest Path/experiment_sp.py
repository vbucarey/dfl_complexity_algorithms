...
import spo_model_sp as spo
import local_search_sp as ls
import penalization_model_sp as pm
import exact_model_sp as ref
import shortest_path as sp
import alternating_sp as alter
import re
import time
...
def experiment(filename, time_limit, cota):
    ...
    for i in range(len(N)):
      real_cost.append(sp.shortestpath(c[i], A, b, E, V)[0].objVal)
    
    #------------------------Obtain initial solutions from SPO+------------------------
    mspo, wspo = spo.spo_mod(filename, 2, real_cost)
    ...
    m_exa, w_exa, iter_omega_exa, iter_time_exa = ref.reformulated(filename, model_time, ls_SPO, real_cost)
    w[sequences[2]] = w_exa
    times[sequences[2]] = m_exa.RunTime
    gap[sequences[2]] = m_exa.MIPGap*100
    f_obj[sequences[2]] = m_exa.ObjVal
    omega_iter[sequences[2]] = iter_omega_exa
    time_iter[sequences[2]] = iter_time_exa
    ...
    inicio_alt = time.time()
    w_alt1, omegas_alt, obj_alt, time_omega1, time_dual1 = alter.iter_alt(10000, filename, model_time, ls_SPO, real_cost, model_time)
    fin_alt = time.time()
    time_alt = fin_alt - inicio_alt
    w[sequences[4]] = w_alt1
