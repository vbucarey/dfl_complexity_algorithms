...
def experiment(filename, time_limit, cota, env):
    ...
    ls_SPO, fobjSPO, omegasSPO = ls.local_search(filename, iterlimit, T, eps, wspo, time_lim_spols - time_spo, env)
    ...
    m_exa, wrefSPO, iter_omega_ref, iter_time_ref = ref.exact(filename, model_time, ls_SPO, real_cost, env)
    ...
    m_pen, w_pen, iter_omega_pen, iter_time_pen = pm.penalization(filename, kappa, model_time, ls_SPO, real_cost, env)
    w[sequences[3]] = w_pen
    ...
    start_alt = time.time()
    w_alt, omegas_alt, obj_alt, time_omega1, time_dual1 = alter.iter_alt(10000, filename, model_time, ls_SPO, real_cost, model_time, env)
    end_alt = time.time()
    ...
    w_alt2, omegas_alt2, obj_alt2, time_omega2, time_dual2 = alter.iter_alt(10000, filename, time_limit - time_spo, wspo, real_cost, time_limit - time_spo, env)
    end_alt2 = time.time()
