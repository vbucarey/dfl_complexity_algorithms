...
def alt_omegafix(filename, timelimit, duals, omega0, realcost, iteration):
...
def alt_dualesfix(filename, timelimit, duals, omega0, costoreal, iteracion):
...
def iter_alt(iterations, filename, timelimit, omega0, realcost, total_time = 3600):
    
    t0 = time.time()
    
    #Initial solution
    alt = alt_omegafix(filename, timelimit, [], omega0, realcost, None)
    ...
    for i in range(iterations):
        if i % 2 == 0:
            alt = alt_dualesfix(filename, timelimit, alt[0], alt[1], realcost, i)
            tiempo_iteracion_dual.append(alt[3])
            
        else:
            alt = alt_omegafix(filename, timelimit, alt[0], alt[1], realcost, i)
            tiempo_iteracion_omega.append(alt[3])
